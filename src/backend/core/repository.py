...
from .models import (
    EnrichedProduct, ProductInDB, ProductSearchQuery, 
    ScraperMetrics, ChipBrand, Store, Price
)
...
class ProductRepository:
    ...
    def create(self, product: EnrichedProduct) -> ProductInDB:
        ...
        if existing:
            ...
            self.session.flush()
            logger.debug("product_updated", product_id=existing.id, url=str(product.url))
            ...
            logger.debug("product_created", product_id=db_product.id, url=str(product.url))
    ...
    def get_best_deals(self, limit: int = 10, chip_brand: Optional[ChipBrand] = None) -> List[ProductInDB]:
        """
    ...
    def get_stats(self) -> dict:
        ...
        by_store = self.session.query(
            Product.store,
            func.count(Product.id)
        ).group_by(Product.store).all()
        
        by_chip = self.session.query(
            Product.chip_brand,
            func.count(Product.id)
        ).group_by(Product.chip_brand).all()
        
        avg_price = self.session.query(
            func.avg(Product.price_value)
        ).filter(Product.price_value > 0).scalar()
        
        min_price = self.session.query(
            func.min(Product.price_value)
        ).filter(Product.price_value > 0).scalar()
        
        max_price = self.session.query(
            func.max(Product.price_value)
        ).filter(Product.price_value > 0).scalar()
        
        latest_scrape = self.session.query(
            func.max(Product.scraped_at)
        ).scalar()
...
class ScraperRunRepository:
    ...
    def get_recent_runs(self, limit: int = 10) -> List[ScraperRun]:
        ...
        return self.session.query(ScraperRun).order_by(
    ...
    def get_run_stats(self, days: int = 7) -> dict:
        ...
        total_runs = self.session.query(func.count(ScraperRun.id)).filter(
            ScraperRun.started_at >= cutoff_date
        ).scalar()
        
        successful_runs = self.session.query(func.count(ScraperRun.id)).filter(
            ScraperRun.started_at >= cutoff_date,
            ScraperRun.success == True
        ).scalar()
        
        total_products = self.session.query(func.sum(ScraperRun.products_saved)).filter(
            ScraperRun.started_at >= cutoff_date
        ).scalar()
        
        avg_execution_time = self.session.query(func.avg(ScraperRun.execution_time)).filter(
            ScraperRun.started_at >= cutoff_date
        ).scalar()
