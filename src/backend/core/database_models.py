...
class Product(Base):
    ...
    store: Mapped[str] = mapped_column(
        SQLEnum(Store, values_callable=lambda x: [e.value for e in x]),
        nullable=False
    )
    
    # Timestamps
    scraped_at: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.now)
...
class ScraperRun(Base):
    ...
    # Status
    success: Mapped[bool] = mapped_column(Integer, default=False)  # SQLite uses INTEGER for BOOLEAN
