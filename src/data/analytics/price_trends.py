...
class PriceTrendsAnalyzer:
    ...
    def get_moving_average(
        self, chip_brand: ChipBrand, window_days: int = 7
    ) -> List[Dict]:
        """
    ...
    def detect_outliers(
        self, chip_brand: Optional[ChipBrand] = None, threshold_std_devs: float = 2.0
    ) -> List[Dict]:
        """
        ...
        logger.info(
            "outliers_detected", count=len(outliers), threshold=threshold_std_devs
        )

        return outliers

    def get_price_trend(self, chip_brand: ChipBrand, days: int = 30) -> str:
        """
