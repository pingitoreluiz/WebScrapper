...
class DataCleaner:
    ...
    def clean_price(price: Price) -> Price:
        ...
        return Price(
            raw=cleaned_raw,
            value=Decimal(str(cleaned_value)),
    ...
    def clean_product(product: ProductInDB) -> ProductInDB:
        ...
        return ProductInDB(
            id=product.id,
            title=DataCleaner.clean_text(product.title),
            price=DataCleaner.clean_price(product.price),
            url=str(product.url).strip() if product.url else product.url,
            store=product.store,
            chip_brand=product.chip_brand,
            manufacturer=DataCleaner.standardize_manufacturer(product.manufacturer),
            model=product.model,
