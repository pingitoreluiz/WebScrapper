...
class DataValidator:
    ...
    @staticmethod
    def validate_required_fields(product: ProductInDB) -> Tuple[bool, Optional[str]]:
        """
    ...
    @staticmethod
    def validate_product(product: ProductInDB, strict: bool = False) -> Tuple[bool, List[str]]:
        """
        Validate a product
        ...
        # Validate title
        if product.title:
            valid, error = DataValidator.validate_title(product.title)
            if not valid:
                errors.append(error)
        
        # Validate price
        if product.price:
            valid, error = DataValidator.validate_price(product.price.value)
            if not valid:
                errors.append(error)
        
        # Validate URL
        if product.url:
            valid, error = DataValidator.validate_url(str(product.url))
            if not valid:
                errors.append(error)
        ...
        for product in products:
            ...
            if is_valid:
                ...
                for error in errors:
                    ...
                    stats["errors_by_type"][category] = stats["errors_by_type"].get(category, 0) + 1
