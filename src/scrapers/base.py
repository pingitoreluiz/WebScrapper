...
from ..backend.core.models import ScraperMetrics, EnrichedProduct, RawProduct, Store, Price
...
class BaseScraper(ABC):
        ...
        try:
            ...
            while self._should_continue(page_num):
                try:
                    ...
                    self.logger.error("unexpected_error", page=page_num, error=str(e), exc_info=True)
                    self.metrics.errors += 1
        ...
        for selector in selectors.product_card:
            try:
                ...
                if len(elements) > 5:  # Reasonable number of products
                    self.logger.debug("using_selector", selector=selector, count=len(elements))
                    return elements
            except:
                continue
    ...
    async def _extract_product_data(self, element) -> ExtractionResult:
        ...
        if selectors.availability:
            result.available = await self._check_availability(element, selectors.availability)
                ...
                if count:
                    ...
                    if href:
                        ...
                        if not href.startswith("http"):
                            ...
                            href = f"https://{self.get_store_name().lower()}.com.br{href}"
                        return href
            except:
                continue
        return None
