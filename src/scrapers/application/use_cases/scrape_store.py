...
class ScrapeStoreUseCase:
    ...
    async def execute(self, config: ScraperConfigDTO) -> ScraperMetricsDTO:
        ...
        try:
            ...
            while page_num <= config.max_pages:
                ...
                # Extract products
                extraction_results = await self.extractor.extract_products(page_content)
                ...
                if not self.extractor.should_continue(page_num, run.products_found):
                    break
    ...
    def _create_product_from_dto(self, dto: ExtractionResultDTO, store: str) -> Product:
        """
