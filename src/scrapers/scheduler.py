...
class ScraperScheduler:
    ...
    async def run_now(self, store: Store, config: Optional[ScraperConfig] = None) -> ScraperMetrics:
        """
    ...
    async def _run_scraper(self, store: Store, config: Optional[ScraperConfig] = None) -> ScraperMetrics:
        """
        ...
        try:
            ...
            logger.error("scraper_execution_failed", store=store.value, error=str(e), exc_info=True)
            return ScraperMetrics(store=store)
