...
class ScraperRegistry:
    ...
    def register(cls, store: Store, scraper_class: Type[BaseScraper]) -> None:
        ...
        logger.debug("scraper_registered", store=store.value, class_name=scraper_class.__name__)
...
class ScraperFactory:
    ...
    def create(store: Store, config: Optional[ScraperConfig] = None) -> BaseScraper:
        ...
        logger.info("creating_scraper", store=store.value, class_name=scraper_class.__name__)
        
        return scraper_class(config)
    
    @staticmethod
    def create_all(config_overrides: Optional[Dict[Store, ScraperConfig]] = None) -> List[BaseScraper]:
        """
