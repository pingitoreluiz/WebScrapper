...
class TestProductEndpoints:
    ...
    def test_search_products(self, client, sample_products):
        ...
        assert len(data["items"]) >= 2  # Should find RTX 4090 and 4080
        assert all("RTX" in item["title"] for item in data["items"])
    ...
    def test_filter_by_chip_brand(self, client, sample_products):
        ...
        assert all(item["chip_brand"] == "NVIDIA" for item in data["items"])
    ...
    def test_filter_by_store(self, client, sample_products):
        ...
        assert all(item["store"] == "Pichau" for item in data["items"])
...
class TestScraperEndpoints:
    ...
    def test_scraper_status(self, client, sample_scraper_run):
