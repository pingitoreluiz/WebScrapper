...
from src.backend.core.models import (
    ProductInDB, ChipBrand, Store, Price, ScraperMetrics
)
...
class TestHealthEndpoints:
    ...
    def test_detailed_health_check(self, mock_get_db, client, mock_db):
        ...
        with patch('src.backend.api.routes.health.ProductRepository', return_value=mock_repo):
...
class TestProductEndpoints:
    ...
    def test_list_products(self, mock_get_db, client, mock_db, sample_product):
        ...
        with patch('src.backend.api.routes.products.ProductRepository', return_value=mock_repo):
...
class TestScraperEndpoints:
    ...
    def test_run_scrapers(self, mock_get_db, mock_get_scheduler, client):
        ...
        response = client.post(
            "/api/v1/scrapers/run",
            json={"stores": ["Pichau"], "headless": True}
        )
    ...
    def test_get_scraper_history(self, mock_get_db, client):
        ...
        with patch('src.backend.api.routes.scrapers.ScraperRunRepository', return_value=mock_repo):
